...
/**
 * 第一版流程图的完善画出，对于各种特别的语句有特别的图形处理，
 * 对于真正的测试流程可不用，以后更迭精细处理
 * @author shaojing
 *
 */
public class IfVisitor extends ASTVisitor {
	 ...
	 * @param b
	 */
	public void branch(Statement b) {
	...
	@Override
	// 有问题，；临时占坑(this is interesting)
	public boolean visit(TryStatement n) {
		Block b = n.getBody();
		List<ASTNode> templist = new ArrayList<>();
		templist = b.statements();
		for (ASTNode a : templist) {
			a.accept(this);
		}
		//catch方块
		List catchlist = n.catchClauses();//body;exceptionDecl
		//finall方块
		Block finallBody = n.getFinally();//
		finallBody.statements();


		return false;
	}
	@Override
	//switch 中 case有特别的类型判断变量类型可以使byte,short,int,char,String
	//在switch中发现规则,然后发现代码都是有固定的规则
	public boolean visit(SwitchStatement n) {
		Expression a = n.getExpression();
		List<ASTNode> blist = n.statements();
		for(ASTNode ast:blist){
			if(ast instanceof SwitchCase){
	...
	public boolean visit(EnhancedForStatement n) {
		...
		// 括号内的list顺序
		if (reversemapping.get(ownid) == null) {
		...
		for (ASTNode node : stateList) {
			MyASTNode eNode = getNode(node);
			forList.add(eNode);
		}

		for (int i = 1; i < forList.size(); i++) {
			MyASTNode prepic = forList.get(i - 1);

			MyASTNode pic = forList.get(i);

			if (pic.astNode instanceof IfStatement || pic.astNode instanceof ForStatement
					|| pic.astNode instanceof EnhancedForStatement || pic.astNode instanceof TryStatement) {
				pic.astNode.accept(this);// 迭代
			}
	...
	public boolean visit(ForStatement n) {
		...
		// 自己大括号的hashcode
		 int ppid = n.getParent().hashCode();
		if (stateList.size() >= 3) {
			ppid = stateList.get(2).getParent().hashCode();
		}
		// 括号的list顺序，刷新括号内顺序

		if (reversemapping.get(ppid) == null) { // 去掉了，感觉哪里赋值错误
			reversemapping.put(ppid, stateList);
		}
		// 记录大括号内的最后语句定义，以if的code为key
		if (lastpoint.get(ownid) == null) {
			// 创建for循环记录
		...
		MyASTNode initializerNode = getNode(initializer.get(0));
		MyASTNode conditionNode = getNode(condition);
		MyASTNode updaterNode = getNode(updater.get(0));
		initializerNode.shape = "record";
		conditionNode.shape = "diamond";

		// ******构建for的循环体格式i=0，i<1，xxxxx ，i++
		List<MyASTNode> forList = new ArrayList<>();
		// forList.add(initializerNode);
		// forList.add(conditionNode);
		// 记录for内的最后语句定义，以for的code为key,条件语句是最后一个
		for (ASTNode node : stateList) {
			MyASTNode eNode = getNode(node);
		...
		// forList.add(updaterNode);

		// 针对初始化和条件做处理
		int parentid = n.getParent().hashCode();
		if (reversemapping.get(parentid) != null) {
			List<Integer> initializerPreIds = new ArrayList<>();
			// for所在大括号内的前置id
			initializerPreIds = getNewPreids(reversemapping.get(parentid), n, initializerPreIds);
			initializerNode.setPreIds(initializerPreIds);
			addlink(initializerNode);// 创建连接

			List<Integer> conditionPreIds = new ArrayList<>();
			MyASTNode lastNode = forList.get(forList.size() - 1);
			conditionPreIds.add(initializerNode.astNode.hashCode());
			conditionPreIds.add(lastNode.astNode.hashCode());
			conditionNode.setPreIds(conditionPreIds);
			addlink(conditionNode);// 创建连接

		}

		for (int i = 2; i < forList.size(); i++) {
			MyASTNode prepic = forList.get(i - 1);
			MyASTNode pic = forList.get(i);
			// 获得前置语句的所有结果集
			...
			int id = prepic.astNode.hashCode();
			preids.addAll(getNormalResultPoint(id, preids));

			pic.setPreIds(preids);
			addlink(pic);
		}
